...
import { Sparkles, Loader2, BookOpen, AlertCircle, Feather, ChevronDown } from 'lucide-react';
...
const KnowledgeCenter = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (window.innerWidth < 1024) {
      window.scrollTo({ top: document.getElementById('ai-results')?.offsetTop ? (document.getElementById('ai-results')!.offsetTop - 100) : 0, behavior: 'smooth' });
    }
  };
  ...
  const renderFormattedContent = (htmlContent: string) => {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-700">
            {/* Header / Title Area */}
            <div className="bg-deep-green text-white p-6 md:p-10 pb-12 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-10 transform translate-x-4 -translate-y-4">
                    <Feather size={140} />
                </div>
                <h2 className="text-2xl sm:text-3xl md:text-4xl font-serif font-medium relative z-10">
                    {ASANA_LIST.find(a => a.sanskrit_name === selectedAsana)?.english_name || 'Asana Guide'}
                </h2>
                <p className="text-sage-green mt-1 text-base sm:text-lg md:text-xl italic relative z-10">
                    {ASANA_LIST.find(a => a.sanskrit_name === selectedAsana)?.sanskrit_name || selectedAsana}
                </p>
            </div>

            {/* Content Body */}
            <div className="p-6 sm:p-8 md:p-12 -mt-8 bg-white rounded-t-[2rem] relative z-20">
                <div 
                    className="
                        text-gray-700 leading-relaxed
                        /* Headings Styling */
                        [&_h3]:font-serif [&_h3]:text-xl sm:[&_h3]:text-2xl [&_h3]:text-deep-green [&_h3]:mt-8 [&_h3]:mb-4 [&_h3]:border-b [&_h3]:border-gray-100 [&_h3]:pb-2
                        /* Paragraphs */
                        [&_p]:mb-4 [&_p]:text-sm sm:[&_p]:text-base [&_p]:leading-7
                        /* Lists */
                        [&_ul]:list-disc [&_ul]:pl-5 [&_ul]:mb-6 [&_ul]:space-y-2
                        [&_li]:pl-1 [&_li]:text-sm sm:[&_li]:text-base
                        /* Strong/Bold text */
                        [&_strong]:font-semibold [&_strong]:text-deep-green
                        /* End Note Box */
                        [&_.end-note]:bg-amber-50 [&_.end-note]:p-5 sm:[&_.end-note]:p-6 [&_.end-note]:rounded-xl [&_.end-note]:border [&_.end-note]:border-amber-100 
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                ...
                <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <form onSubmit={handleGenerate} className="space-y-5">
                        <div className="text-left">
                            <label className="block text-xs font-bold text-gray-700 mb-2 uppercase tracking-widest">Asana Category</label>
                        ...
                        <div className="text-left">
                            <label className="block text-xs font-bold text-gray-700 mb-2 uppercase tracking-widest">Asana Name</label>
                            <div className="relative">
                                <select 
                                    value={selectedAsana}
                        ...
                        <button 
                            ...
                            className="w-full bg-deep-green text-white py-4 rounded-xl hover:bg-opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2 font-medium text-base shadow-md shadow-deep-green/10 mt-2 transform active:scale-[0.98]"
                        >
                            {loading ? <Loader2 className="animate-spin" size={20} /> : <Sparkles size={20} />}
                            {loading ? 'Consulting AI...' : 'Generate Guide'}
                        </button>
                    </form>
                </div>
                
                <div className="bg-sage-green/10 p-5 rounded-xl border border-sage-green/20 text-deep-green text-sm flex gap-3 text-left">
            ...
            <div id="ai-results" className="lg:col-span-8 min-h-[400px] sm:min-h-[600px]">
                {loading ? (
                    <div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-6 bg-white rounded-3xl border border-gray-100 shadow-sm p-8 sm:p-12 min-h-[400px]">
                        <div className="relative">
                            <div className="w-16 h-16 sm:w-20 sm:h-20 border-4 border-sage-green/30 border-t-deep-green rounded-full animate-spin"></div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                <Sparkles size={24} className="text-sage-green animate-pulse" />
                            </div>
                        </div>
                        <div className="text-center">
                            <p className="text-lg sm:text-xl font-serif text-deep-green mb-2">Consulting the ancient texts...</p>
                            <p className="text-sm sm:text-base text-gray-400">Curating detailed knowledge for your practice...</p>
                        </div>
                    </div>
                ) : generatedContent ? (
                    <div className="pb-10">
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-gray-400 bg-white rounded-3xl border border-dashed border-gray-200 p-8 sm:p-12 text-center min-h-[400px]">
                        <div className="w-20 h-20 sm:w-24 sm:h-24 bg-gray-50 rounded-full flex items-center justify-center mb-6">
                            <BookOpen size={40} className="text-gray-300" />
                        </div>
                        <h3 className="text-xl sm:text-2xl font-serif text-gray-600 mb-3">Ready to Learn?</h3>
                        <p className="max-w-md text-sm sm:text-base text-gray-500 leading-relaxed">
